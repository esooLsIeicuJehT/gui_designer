...
def sanitize_python_identifier(raw_name: Any, fallback: str, used_names: set[str]) -> str:
    text = str(raw_name or "").strip()
    sanitized = re.sub(r"\W", "_", text)
    if not sanitized or sanitized[0].isdigit():
        sanitized = f"_{sanitized}" if sanitized else ""
    if not sanitized:
        sanitized = fallback
...
class PropertyEditor(QWidget):
    ...
    def _code_tab(self):
        ...
        row = QHBoxLayout()
        self.generate_target = QComboBox()
        self.generate_target.addItems(["PyQt6", "ImGui", "Generic GUI"])
        ...
        self.generate_target.currentTextChanged.connect(self.main_window.on_design_change)

        row.addWidget(self.generate_target)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ScriptForge Studio")
    ...
    def load_script(self, path: str):
        try:
            ...
            QMessageBox.critical(self, "Load Error", f"Unable to load script:\n{exc}")
    ...
    def save_project(self):
        path, _ = QFileDialog.getSaveFileName(self, "Save Project", "", "ScriptForge Studio Project (*.sfs.json)")
        if not path:
            return
        try:
            ...
            QMessageBox.critical(self, "Save Error", f"Unable to save project:\n{exc}")

    def load_project(self):
        path, _ = QFileDialog.getOpenFileName(self, "Load Project", "", "ScriptForge Studio Project (*.sfs.json)")
        if not path:
            return
        try:
            ...
            QMessageBox.critical(self, "Load Error", f"Unable to load project:\n{exc}")
    ...
    def export_template(self, fmt: str):
        ...
        if fmt == "json":
            ...
            if path:
                try:
                    ...
                    QMessageBox.critical(self, "Export Error", f"Unable to export JSON template:\n{exc}")
            return
        ...
        if path:
            try:
                ...
                QMessageBox.critical(self, "Export Error", f"Unable to export YAML template:\n{exc}")
    ...
    def import_template(self, fmt: str):
        if fmt == "json":
            ...
            try:
                ...
                QMessageBox.critical(self, "Import Error", f"Unable to import JSON template:\n{exc}")
                return
        else:
            if not yaml_available():
            ...
            try:
                ...
                QMessageBox.critical(self, "Import Error", f"Unable to import YAML template:\n{exc}")
                return
    ...
    def refresh_code_preview(self):
        target = self.editor.generate_target.currentText()
        code = self.generate_gui_code(self.editor.design(), self.original_code, target)
    ...
    def generate_merged_script(self, output_path: str):
        try:
            ...
            QMessageBox.information(self, "Saved", f"{target} script saved to:\n{output_path}")
        except Exception as exc:
            QMessageBox.critical(self, "Save Error", f"Unable to save generated script:\n{exc}")

    def export_ui_file(self):
        if self.editor.generate_target.currentText() != "PyQt6":
            QMessageBox.warning(self, "Unsupported target", ".ui export is only supported for PyQt6 target.")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Export Error", f"Unable to export .ui file:\n{exc}")

    def run_smoke_test(self):
        try:
            generated = self.generate_gui_code(self.editor.design(), self.original_code, self.editor.generate_target.currentText())
            ...
            QMessageBox.critical(self, "Smoke Test Error", str(exc))

    def generate_prompt_result(self, prompt: str, framework: str) -> str:
        if framework == "PyQt6":
            return self.generate_gui_code(self.editor.design(), self.original_code, "PyQt6")
        if framework == "ImGui":
            return self.generate_gui_code(self.editor.design(), self.original_code, "ImGui", prompt=prompt)
        return self.generate_gui_code(self.editor.design(), self.original_code, "Generic GUI", prompt=prompt)

    def generate_gui_code(self, design: dict[str, Any], original_code: str, target: str = "PyQt6", prompt: str = "") -> str:
        if target == "ImGui":
            escaped_prompt = prompt.replace("'", "\\'")
            return f'''"""ImGui generated output
Prompt: {escaped_prompt}
"""
ORIGINAL_SCRIPT = {original_code!r}

# Requires: pip install pyimgui glfw
import imgui
...
def draw_ui():
    imgui.begin({design['window_title']!r})
    imgui.text({WATERMARK_TEXT!r})
    imgui.text_wrapped({escaped_prompt!r} or 'Generated layout prompt')
    if imgui.button('Run Original Script'):
        run_original()
    imgui.end()
'''
        if target == "Generic GUI":
            return (
                f"# Generic GUI scaffold\\n"
                f"# Window: {design['window_title']}\\n"
                f"# Prompt: {prompt or 'N/A'}\\n"
                f"# Sections: header, content, actions\\n"
                f"# Widgets: {[w['type'] for w in design['widgets']]}\\n"
        ...
        widget_identifiers = [
            sanitize_python_identifier(w.get("name", ""), f"widget_{i + 1}", used_identifiers)
            for i, w in enumerate(widgets)
        ]
        for i, w in enumerate(widgets):
            n = widget_identifiers[i]
        ...
        connect_lines = [
            f"self.{widget_identifiers[i]}.clicked.connect(self.run_original)"
            for i, w in enumerate(widgets)
            if w["type"] == "Button"
        ]

        return textwrap.dedent(f'''"""
Generated by ScriptForge Studio
